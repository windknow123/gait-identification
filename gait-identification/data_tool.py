...
def load_training_validation_data(train_view=None, val_view=None, train_dir=None, val_dir=None):
    human_id = ["%03d" % i for i in range(1, 21)]
    ...
    for id in human_id:
        ...
        for dir in train_dir:
            ...
            if len(data.shape) > 0:
				...
				#imsave("%s/train-%s-%s-%s-GEI.bmp" % (config.Project.casia_test_data_path, id, dir, train_view))
            else:
        ...
        for dir in val_dir:
            ...
            if len(data.shape) > 0:
				...
				#imsave("%s/val-%s-%s-%s-GEI.bmp" % \
				#	(config.Project.casia_test_data_path, id, dir,\
				#	val_view))
            else:
            	logger.warning("fail to extract %s of %s" % (img_dir, id))
...
if __name__ == '__main__':
	...
	print("training_y.len: ", len(training_y))
